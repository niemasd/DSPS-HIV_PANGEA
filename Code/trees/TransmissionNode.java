 ...
 * @version 1 Nov 2013 - added parameter viralLoad, this is set to 0 if Host passed in in constructor has no Virus. Added getNexusName which adds VL onto name returned when making Nexus Trees
 * @version 5 Nov 2013 - Now can delete multiple one-child nodes from a branch
 */
public class TransmissionNode implements Node, Cloneable {
	...
	protected double		viralLoad;
	protected String		migStat;
	...
	TransmissionNode(Host host) {
		...
		if(host.getVirus() != null){
			viralLoad = host.getVirus().getViralLoad();
	...
	*/
	...
	public String getNexusName(){
		if(migStat != null)
			return name+"_"+migStat+"[&vl="+viralLoad+"]";
		return name+"[&vl="+viralLoad+"]";
	}
	...
	*/
	...
	/**
	 * 
	 * This allows the program to work down a branch and 'delete' multiple single-child nodes.
	 * Rather than just deleting 2 single-child nodes from each branch.
	 * E Hodcroft - 4 Nov 2013
	 */
			...
			for (Node child : children) {
				...
				*/
				if(child.getNumberOfChildren()==1){
					...
					while(grandChild.getNumberOfChildren() == 1){ //will stop at 2 or 0! (has more children, or is tip)
						grandChild = grandChild.getChildren().get(0);
					}
					
					while(grandChild.getParent() != this){
						grandChild.setParent(grandChild.getParent().getParent());
